var CoindDeskWidget = function() {
        var a = this;
        a._protocol = window.location.protocol, a.initialize = function() {
            window.jQuery && (a._widget = document.getElementById("coindesk-widget"), null === a._widget && alert('Please ensure you have included the tag where you want the widget to appear.  Using the markup: <div id="coindesk-widget"></div>'), a._widget.layout = a._widget.getAttribute("data-size"), a._widget.layout || console.warn("No layout specified by attribute 'data-size', defaulting to 'small'. Supports 'mpu'(300x250) and 'small'(210x130). For example: <div id=\"coindesk-widget\" data-size=\"mpu\"></div>"), a._widget.alignment = a._widget.getAttribute("data-align"), a._widget.alignment || console.warn("No alignment specified by attribute 'data-align', defaulting to 'left'. Supports 'left', 'center' and 'right'."), a._modernBrowsers = !(jQuery.browser.msie && jQuery.browser.versionNumber <= 8), a.getDimensions(a._widget.layout), a.getAlignment(a._widget.alignment), a.createTemplate(), a.createWidget(), a.getHistoryData(), a.getCurrentData())
            ...
            if (a._modernBrowsers) c = i, b = f;
            else switch (a._widget.config.name) {
            case "mpu":
                c = h, b = e;
                break;
            case "small":
        ...
        a.ieLoadBugFix = function(c, d) {
            ...
            b > c ? (jQuery(a._elements.primary).addClass("delta-down"), jQuery(a._elements.primary).addClass("fadeInDown")) : c > b ? (jQuery(a._elements.primary).addClass("delta-up"), jQuery(a._elements.primary).addClass("fadeInUp")) : a.resetColor(), setTimeout(function() {
                a.resetColor()
            }, 1e3)
        }, a.resetColor = function() {
            jQuery(a._elements.primary).removeClass("fadeInUp"), jQuery(a._elements.primary).removeClass("fadeInDown"), jQuery(a._elements.primary).removeClass("delta-up"), jQuery(a._elements.primary).removeClass("delta-down")
        }, a.updateSparkline = function() {
            var b, c, d, e = "#sparkline",
                f = a._widget.config.spark.width + 20,
                g = a._widget.config.spark.height,
                h = "cardinal",
                i = 1e3;
            if (a._graph || (a._graph = d3.select(e).append("svg:svg").attr("width", "100%").attr("height", "100%")), a._history.length > 60 && a._history.shift(), b = d3.scale.linear().domain([0, 60]).range([-2, f]), c = d3.scale.linear().domain([d3.max(a._history) + 1, d3.min(a._history) - .3]).range([0, g]), d = d3.svg.line().x(function(a, c) {
                return b(c)
            }).y(function(a) {
                return c(a)
            }).interpolate(h), a._graph.selectAll(".btc-path").remove(), a._graph.selectAll(".focus").remove(), a._graph.selectAll(".overlay").remove(), a._graph.append("svg:path").attr("d", d(a._history)).attr("class", "line"), a._graph.selectAll("path").data([a._history]).attr("transform", "translate(" + b(1) + ")").attr("d", d).attr("class", "btc-path").transition().ease("linear").duration(i).attr("transform", "translate(" + b(0) + ")"), a._widget.config.spark.tooltip) {
                var j = function(a, b) {
                        var c = "$" + a.toFixed(2) + " (" + (57 - b) + "mins ago)";
                        return c
                    },
                    k = function() {
                        var d = b.invert(d3.mouse(this)[0]),
                            e = Math.floor(d),
                            f = a._history[e + 1];
                        l.select("circle").attr("transform", "translate(" + b(d) + "," + c(f) + ")"), l.select("text").text(j(f, e)), l.select("line").attr("transform", "translate(" + b(d) + ", 0)").attr("y1", g)
                    },
                    l = a._graph.append("g").attr("class", "focus").style("display", "none");
                l.append("circle").attr("r", 6), l.append("line").attr(), a._widget.config.spark.tooltipBg && l.append("rect").attr("class", "tooltipBg").attr("transform", "translate(-10,0)").attr("width", 153).attr("height", 22).attr("rx", 4).attr("ry", 4), l.append("text").attr("transform", "translate(12,15)"), a._graph.append("rect").attr("class", "overlay").attr("width", f).attr("height", g).on("mouseover", function() {
                    l.style("display", null)
                }).on("mouseout", function() {
                    l.style("display", "none")
                }).on("mousemove", k)
            }
        }, a.addScript(a._protocol + "//widget.coindesk.com/bpiticker/jquery.ajax.only.d3.min.js", a.initialize)
    },
    _widget = new CoindDeskWidget;